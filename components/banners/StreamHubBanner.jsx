...
export default function StreamHubBanner({ onClick, href }) {
  ...
  const W = href ? 'a' : 'div';
  return (
    <W href={href} onClick={onClick} style={{display:'flex',alignItems:'center',gap:'14px',padding:'1rem 1.25rem',background:'var(--color-background-primary)',border:'.5px solid var(--color-border-tertiary)',borderLeft:'3px solid #6B21A8',borderRadius:'12px',overflow:'hidden',position:'relative',cursor:'pointer',textDecoration:'none',transition:'border-left-color .2s'}}
      onMouseEnter={e=>e.currentTarget.style.borderLeftColor='#9333EA'}
      onMouseLeave={e=>e.currentTarget.style.borderLeftColor='#6B21A8'}>
