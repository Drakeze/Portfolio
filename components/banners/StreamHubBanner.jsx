...
export default function StreamHubBanner({ onClick, href }) {
  ...
  useEffect(() => {
    ...
    BAR_SCALES.forEach((h, i) => {
      ...
      b.style.cssText = `width:3px;height:52px;flex-shrink:0;background:#C94C2A;border-radius:2px 2px 0 0;transform-origin:bottom;transform:scaleY(${h});animation:streamWave ${dur}s ${i*.06}s ease-in-out infinite;--mn:${mn}`;
      bars.appendChild(b);
    });
  }, []);
  useEffect(() => {
  ...
  const W = href ? 'a' : 'div';
  return (
    <W href={href} onClick={onClick} style={{display:'flex',alignItems:'center',gap:'14px',padding:'1rem 1.25rem',background:'var(--color-background-primary)',border:'.5px solid var(--color-border-tertiary)',borderLeft:'3px solid #C94C2A',borderRadius:'12px',overflow:'hidden',position:'relative',cursor:'pointer',textDecoration:'none',transition:'border-left-color .2s'}}
      onMouseEnter={e=>e.currentTarget.style.borderLeftColor='#D85A30'}
      onMouseLeave={e=>e.currentTarget.style.borderLeftColor='#C94C2A'}>
      <div style={{flexShrink:0,width:'80px',height:'78px',position:'relative',display:'flex',flexDirection:'column',justifyContent:'flex-end'}}>
        <div style={{position:'absolute',top:'5px',left:0,display:'flex',alignItems:'center',gap:'4px'}}>
          <div style={{width:'6px',height:'6px',borderRadius:'50%',background:'#D85A30',animation:'liveDot 1.1s ease-in-out infinite'}}/>
          <span style={{fontSize:'9px',letterSpacing:'1.5px',color:'#C94C2A',fontFamily:'monospace',fontWeight:500}}>LIVE</span>
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontSize:'20px',fontWeight:500,color:'var(--color-text-primary)',lineHeight:1,marginBottom:'4px'}}>STREAM<span style={{color:'#C94C2A'}}>HUB</span></div>
        <div style={{fontSize:'10px',letterSpacing:'2px',textTransform:'uppercase',color:'#C94C2A',fontFamily:'monospace',marginBottom:'10px'}}>Live · On Demand</div>
